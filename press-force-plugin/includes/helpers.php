...
class SalesforceAPI extends APIAbstract {
     ...
     */
    public function __construct($instance_url,$version, $client_id, $client_secret, $redirect_uri, $return_type = self::RETURN_OBJECT )
    {
        // Instantiate base variables
        $this->instance_uri = $instance_url;
...
//        }
	...
	public function getAccessToken($refresh_token)
	{
		...
		$handle = curl_init($this->base_url . '/services/oauth2/token');
		curl_setopt($handle, CURLOPT_HEADER, FALSE);
		curl_setopt($handle, CURLOPT_RETURNTRANSFER, TRUE);
		curl_setopt($handle, CURLOPT_POST, TRUE);
		curl_setopt($handle, CURLOPT_POSTFIELDS, $params);
	    curl_setopt($handle, CURLOPT_SSL_VERIFYPEER, TRUE);
		// curl_setopt($handle, CURLOPT_CAINFO, plugin_dir_path(__FILE__)."inc/cacert.pem");
		...
		if ( $status != 200 ) 
		{
     		$response 	= array(
     			'status'		=>	$status,
     			'msg' 			=>	"Error: call to URL failed with status $status",
    ...
    public function loadMore($path, $params = [], $method = self::METH_GET, $headers = [])
    {
        ...
        // echo $this->instance_uri  . $path;

        return $this->httpRequest($this->instance_uri . $path, $params, $request_headers, $method);
    }

    public function getsObject($path, $params = [], $method = self::METH_GET, $headers = [])
    {
        // Throw an error if no access token
        if(!isset($this->access_token))
            throw new SalesforceAPIException('You have not logged in yet.');
